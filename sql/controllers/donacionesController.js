...
class DonacionesController {
    ...
    static async create(req, res) {
        try {
            const { tipo_donacion, fecha_donacion, id_donante, id_campana } = req.body;
            const id_donacion = await DonacionesModel.create(tipo_donacion, fecha_donacion, id_donante, id_campana);
            
            res.status(201).json({ message: 'Donación creada', id_donacion });
        } catch (error) {
