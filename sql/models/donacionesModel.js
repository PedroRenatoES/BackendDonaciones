...
class DonacionesModel {
    ...
    static async create(tipo_donacion, fecha_donacion, id_donante, id_campana, estado_validacion = 'pendiente') {
        try {
            const pool = await poolPromise;
            ...
            if (tipo_donacion === 'especie') {
                const especieResult = await pool.request()
                    ...
                    .input('cantidad', sql.Int, null)
                    .input('estado_articulo', sql.VarChar(100), 'sin asignar')
                    .input('destino_donacion', sql.Text, 'sin asignar')
                    .query(`
                        INSERT INTO DonacionesEnEspecie (id_donacion, id_articulo, id_espacio, cantidad, estado_articulo, destino_donacion)
                        VALUES (@id_donacion, @id_articulo, @id_espacio, @cantidad, @estado_articulo, @destino_donacion);
                ...
                const dineroResult = await pool.request()
                    ...
                    .query(`
                        INSERT INTO DonacionesEnDinero (id_donacion, monto, divisa, nombre_cuenta, numero_cuenta, comprobante_url)
                        VALUES (@id_donacion, @monto, @divisa, @nombre_cuenta, @numero_cuenta, @comprobante_url);
